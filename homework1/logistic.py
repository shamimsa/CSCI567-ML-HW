...
def logistic_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for iter in range(max_iterations):
        ...
        w_aug = w_aug - step_size * (grad_sum / num_train)
    b = w_aug[0][0]
    #print(b)
    w = w_aug[0][1:]
    #print(w)
...
def logistic_test(Xtest, ytest, w, b, t = 0.5):
    ...
    for i in range(num_test):
        ...
        pred = 1/ (1 + numpy.exp(- pred))
        if pred >= t:
            ypred.append(1)
        else:
            ypred.append(0)
    ypred = numpy.asarray(ypred)
    correct = 0
    for iter in range(num_test):
        if ypred[iter] == ytest[iter]:
            correct += 1
...
def feature_square(Xtrain, Xtest):
    ...
    for i in range(len(Xtrain_s)):
        for j in range(len(Xtrain_s[0])):
            Xtrain_s[i][j] = Xtrain[i][j] ** 2
    for i in range(len(Xtest_s)):
        for j in range(len(Xtest_s[0])):
            Xtest_s[i][j] = Xtest[i][j] ** 2
