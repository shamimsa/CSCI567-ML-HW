...
def compute_distances(Xtrain, X):
    ...
    for i in range(len(X)):
        for j in range(len(Xtrain)):
            dist = np.linalg.norm(X[i][:] - Xtrain[j][:])
            dists[i][j] = dist
    #####################################################
    return dists
...
def predict_labels(k, ytrain, dists):
    ...
    for i in range(num_test):
        test_dists = dists[i]
        smallest_dist_idx = np.argsort(test_dists)[:k]
        #print(smallest_dist_idx)
        pred_labels = [ytrain[idx] for idx in smallest_dist_idx]
        #print(pred_labels)
        unique_labels, label_counts = np.unique(pred_labels, return_counts=True)
        majority_tuples = zip(unique_labels, label_counts)
        labels_sorted = sorted(majority_tuples, key= lambda x: (-x[1], x[0]))
        #print(labels_sorted)
...
def find_best_k(K, ytrain, dists, yval):
    ...
    for k in K:
        ...
        pred_acc = compute_accuracy(yval, ypred)
        validation_accuracy.append(pred_acc)
    best_k_idx = np.argmax(validation_accuracy)
    best_k = K[best_k_idx]
	#####################################################
