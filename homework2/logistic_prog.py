...
def logistic_train_ovr(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    N, D = Xtrain.shape
    aug_dim = D + 1
    w_aug = numpy.insert(w, 0, b)
    for iter in range(max_iterations):
        grad_sum = numpy.zeros((1,aug_dim))
        for i in range(N):
            train_data = Xtrain[i][:]
            train_aug = numpy.insert(train_data, 0, 1)
            pred_prob = 1/ (1+ numpy.exp(- numpy.dot(train_aug, numpy.transpose(w_aug))))
            grad = (pred_prob - ytrain[i]) * train_aug
            grad_sum = numpy.add(grad_sum, grad)
        w_aug = w_aug - step_size * (grad_sum / N)
    b_l = w_aug[0][0]
    w_l = w_aug[0][1:]
    return w_l, b_l
...
def logistic_test_ovr(Xtest, w_l, b_l):
    ...
    test_pred = numpy.zeros((num_test,))
    w_aug = numpy.insert(w_l,0,b_l)
    for i in range(num_test):
        test_data = numpy.insert(Xtest[i][:], 0, 1)
        pred = numpy.dot(test_data,numpy.transpose(w_aug))
        test_pred[i] = 1/ (1 + numpy.exp(- pred))
    return test_pred
...
def logistic_mul_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for iter in range(max_iterations):
        for c in range(C):
            w_c = w[c][:]
            w_c_aug = numpy.insert(w_c, 0, b[c])
            grad_sum = numpy.zeros((1,aug_dim))
            for i in range(N):
                train_data = Xtrain[i][:]
                train_aug = numpy.insert(train_data, 0, 1)
                pred_prob = 1/ (1+ numpy.exp(- numpy.dot(train_aug, numpy.transpose(w_c_aug))))
                grad = (pred_prob - ytrain[i][c]) * train_aug
                grad_sum = numpy.add(grad_sum, grad)
            w_c_aug = w_c_aug - step_size * (grad_sum / N)
            b_l[c] = w_c_aug[0][0]
            w_l[c][:] = w_c_aug[0][1:]
    return w_l, b_l
...
def logistic_mul_test(Xtest, w_l, b_l):
    ...
    num_test = Xtest.shape[0]
    C = 10
    test_pred = numpy.zeros((num_test,C))
    for c in range(C):
        w_c = w_l[c][:]
        w_c_aug = numpy.insert(w_c, 0, b_l[c])
        for i in range(num_test):
            test_data = numpy.insert(Xtest[i][:], 0, 1)
            pred = numpy.dot(test_data,numpy.transpose(w_c_aug))
            test_pred[i][c] = 1/ (1 + numpy.exp(- pred))
    return test_pred
