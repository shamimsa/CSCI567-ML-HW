...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        D_out = self.params['W'].shape[1]
        N = X.shape[0]
        forward_output = np.zeros((N,D_out))
        #print(X.shape)
        #print(self.params['W'].shape)
        #print(self.params['b'].shape)
        for i in range(len(X)):
            forward_output[i][:] = np.dot(X[i][:],self.params['W']) + self.params['b']
    ...
    def backward(self, X, grad):
        ...
        ##########################################################################################################################
        N,_ = X.shape
        self.gradient['b'] = np.dot(np.ones((1,N)),grad)
